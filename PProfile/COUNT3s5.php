                    ...
                    <table class="table table-striped table-bordered templatemo-user-table">
                        ...
                        <tbody>
                            ...
                            if (isset($_POST['s5'])) {
                                ...
                                $stmt = $conn->prepare("SELECT COUNT(*) FROM basicdetails WHERE `Approve`='1' AND SSLC >= ?");
                                $stmt->bind_param("d", $Csslc);
                                $stmt->execute();
                                $stmt->bind_result($count);
                                $stmt->fetch();
                                $stmt->close();

                                echo "<br><h3>Students Scored Above '$Csslc' in SSLC: $count</h3>";

                                // Prepared statement to select students
                                $stmt = $conn->prepare("SELECT * FROM basicdetails WHERE `Approve`='1' AND SSLC >= ?");
                                $stmt->bind_param("d", $Csslc);
                                $stmt->execute();
