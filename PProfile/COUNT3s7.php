                    ...
                    <table class="table table-striped table-bordered templatemo-user-table">
                        ...
                        <tbody>
                            ...
                            if (isset($_POST['s7'])) {
                                ...
                                $stmt = $conn->prepare("SELECT COUNT(*) FROM basicdetails WHERE `Approve`='1' AND BE >= ?");
                                $stmt->bind_param("d", $Cbe);
                                $stmt->execute();
                                $stmt->bind_result($count);
                                $stmt->fetch();
                                $stmt->close();

                                echo "<br><h3>Students Scored Above '$Cbe' in BE Aggregate: $count</h3>";

                                // Prepared statement to select students
                                $stmt = $conn->prepare("SELECT * FROM basicdetails WHERE `Approve`='1' AND BE >= ?");
                                $stmt->bind_param("d", $Cbe);
                                $stmt->execute();
